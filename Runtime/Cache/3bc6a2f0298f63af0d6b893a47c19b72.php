			...
			$.post('__URL__/myCircle',{},function(data){
				...
				$.each(data,function(index,content){
					if(content.isCreater=='1'){
						$('.toolbar').append("<div class='group group-center group-center-blue' style=''>"+content.circlename+"<i class='icon-wrench icon-white' style='float:right; cursor:pointer; margin-top:3px;' onmouseout='$.doCirMngOut(this)' onmouseover='$.doCirMngOver(this)' onclick=\"$.doCirMng("+content.circleid+","+content.count+",'"+content.time+"',this)\"></i></div>");
					}else{
								...
								if(data == null){
									...
									$.each(data,function(index,content){
										...
										if(content.isJo == 1){
											...
											var isJo = "<button  style='font-size:12px;' type='button' class='btn btn-info circle-"+content.id+"' onclick='$.doJoin("+content.id+",this,1)'>加入圈子</button>";
										}else{
											var isJo = "<button  style='font-size:12px;' type='button' class='btn btn-info circle-"+content.id+"'' onclick='$.doJoin("+content.id+",this,2)'>加入圈子</button>";
										}
					                ...
					                <div class="thumbnail">
					                  <div class="caption">
					                    ...
					                    <p class="join_btn" style="text-align:right;margin:0px; font-size:13px; font-weight:bold">
					                    	<?php if($vo["isJo"] == 1 ): ?><div style='text-align:right;font-size:13px; font-weight:bold;'>已加入</div><?php else: if(($vo["password"]) != ""): ?><button id='circle-<?php echo ($vo["id"]); ?>' style='font-size:12px;' type="button" class='btn btn-info circle-<?php echo ($vo["id"]); ?>' data-toggle='modal' onclick='$.doJoin(<?php echo ($vo["id"]); ?>,this,1)'>加入圈子</button><?php else: ?><button id='circle-<?php echo ($vo["id"]); ?>' style='font-size:12px;' type='button' class='btn btn-info circle-<?php echo ($vo["id"]); ?>' onclick='$.doJoin(<?php echo ($vo["id"]); ?>,this,2)'>加入圈子</button><?php endif; endif; ?>
					                    </p>
					                  </div>
					                </div>
